...
const TWIST_PRESETS: Record<
  string,
  { twist: number; lKnee: number; rKnee: number }
> = {
...
const HAND_PULL_OPTIONS: PullDirection[] = [
  "down",
  "side",
  "undercling",
  "gaston",
...
const FOOT_PULL_OPTIONS: PullDirection[] = [
  "edge",
  "smear",
  "toe-hook",
  "heel-hook",
...
export function segmentToWorld(
...
): { pos: [number, number, number]; angleDeg: number } {
  let curWorldY = 0;
  let curWorldZ = 0;
  let remaining = wallY;
  ...
  return {
    pos: [
      x,
      curWorldY + remaining * Math.cos(angleRad),
      curWorldZ + remaining * Math.sin(angleRad),
    ],
...
const ROUTE_PRESETS: RoutePreset[] = [
  ...
  },
  {
    ...
    wallAngle: 80,
    segments: [
      { height: 1.5, angleDeg: 0 },
      { height: 1.5, angleDeg: 80 },
      { height: 2, angleDeg: 0 },
    ],
    holds: [
  ...
  },
  {
    ...
    wallAngle: 60,
    segments: [
      { height: 1.0, angleDeg: 60 },
      { height: 1.5, angleDeg: 30 },
      { height: 2.5, angleDeg: 0 },
    ],
    holds: [
  ...
  {
    name: "Slab to Overhang",
    grade: "V3",
    wallAngle: -10,
    segments: [
      { height: 2, angleDeg: -10 },
      { height: 1.5, angleDeg: 35 },
      { height: 1.5, angleDeg: 0 },
    ],
    holds: [
  ...
  },
  {
    ...
    wallAngle: 20,
    segments: [
      { height: 2, angleDeg: 20 },
      { height: 1, angleDeg: 70 },
      { height: 2, angleDeg: 10 },
    ],
    holds: [
...
function Slider({
  ...
  return (
    ...
    >
      <label style={{ width: 70, fontSize: 11, color: "#aaa", flexShrink: 0 }}>
        {label}
      </label>
      <input
        type="range"
...
const DEFAULT_STATE: ClimberState = {
  ...
  leftHandPull: "down",
  rightHandPull: "down",
  leftFootPull: "edge" as PullDirection,
  rightFootPull: "edge" as PullDirection,
  leftKneeTurnDeg: 0,
  rightKneeTurnDeg: 0,
  hipOffset: 0.35,
  torsoOffset: 0.5,
...
function decodeRoute(
  encoded: string,
): { segments: WallSegment[]; holds: PlacedHold[] } | null {
  try {
    const data: SharedRoute = JSON.parse(atob(encoded));
    const holds: PlacedHold[] = data.h.map(
      ([x, y, type, direction, usage]) => ({
        id: makeHoldId(),
        x,
        y,
...
function MainPage({ onMultiPitch }: { onMultiPitch: () => void }) {
  ...
  // Funny climbing idiom messages
  const climbingIdioms = [
    "Stay in School",
    "Just go up",
    "Send it!",
    "Next one is a jug",
    ...
    "Should've chalked up",
    "Climb with your feet!",
    "Just dyno it",
  ];
  const [climbingMessage, setClimbingMessage] = useState<string | null>(null);
  const messageTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const showClimbingMessage = useCallback(() => {
    const msg =
      climbingIdioms[Math.floor(Math.random() * climbingIdioms.length)];
    setClimbingMessage(msg);
    if (messageTimerRef.current) clearTimeout(messageTimerRef.current);
    messageTimerRef.current = setTimeout(() => setClimbingMessage(null), 3000);
  }, []);

  const buildExplorerRagdoll = useCallback((): RagdollPart[] => {
    const scale = state.heightFt / 5.75;
    const x = explorerPos.x;
    const z = explorerPos.z;
    const baseY = 0.5;
    ...
    return [
      {
        shape: "sphere",
        color: skinColor,
        size: [0.065 * scale, 0, 0],
        position: [x, baseY + 0.77 * scale, z],
        velocity: rndV(3, 4),
        rotation: [0, 0, 0],
        angularVel: rndAng(8),
      },
      {
        shape: "cylinder",
        color: torsoColor,
        size: [0.09 * scale, 0.28 * scale, 0],
        position: [x, baseY + 0.57 * scale, z],
        velocity: rndV(2, 3),
        rotation: [0, 0, 0],
        angularVel: rndAng(5),
      },
      {
        shape: "cylinder",
        color: limbColor,
        size: [0.02 * scale, 0.18 * scale, 0],
        position: [x - 0.1, baseY + 0.5 * scale, z],
        velocity: rndV(2, 5),
        rotation: [0, 0, 0],
        angularVel: rndAng(10),
      },
      {
        shape: "cylinder",
        color: limbColor,
      ...
      },
      {
        shape: "cylinder",
        color: legColor,
        size: [0.025 * scale, 0.22 * scale, 0],
        position: [x - 0.05, baseY + 0.22 * scale, z],
        velocity: rndV(1, 3),
        rotation: [0, 0, 0],
        angularVel: rndAng(8),
      },
      ...
      },
      {
        shape: "box",
        color: shoeColor,
        size: [0.035 * scale, 0.025 * scale, 0.07 * scale],
        position: [x - 0.04, baseY + 0.01, z],
        velocity: rndV(2, 4),
        rotation: [0, 0, 0],
        angularVel: rndAng(10),
      },
  ...
  const [ragdollCameraTarget, setRagdollCameraTarget] = useState<
  ...
  const handleOutOfBounds = useCallback(() => {
    ...
    const parts = buildExplorerRagdoll();
    setRagdollParts(parts);
    setRagdollCameraTarget(target);
    setIsExploring(false);
    setTimeout(() => {
      setRagdollParts(undefined);
      setRagdollCameraTarget(undefined);
      setIsExploring(true);
    }, 3000);
  }, [showClimbingMessage, buildExplorerRagdoll]);

  // Simulation
  const [isPlaying, setIsPlaying] = useState(false);
  const [isFalling, setIsFalling] = useState(false);
  const [fallReason, setFallReason] = useState<"reach" | "grip">("reach");
  const [ragdollParts, setRagdollParts] = useState<RagdollPart[] | undefined>(
    undefined,
  );
  ...
  const allHoldsOnWall = useMemo(
    () => [
      ...(toppedOut || sittingOnGround || isPlaying
        ? []
        : [startHolds.leftHand, startHolds.rightHand]),
      ...placedHolds,
    ],
  ...
  const buildRagdoll = useCallback((s: ClimberState): RagdollPart[] => {
    ...
    const chestW = toWorld(cogX, cogY + 0.3 * scale, hipOff * 1.5);
    const headW = toWorld(cogX, cogY + 0.4 * scale, hipOff * 1.4);
    ...
    return [
      ...
      // Left upper arm
      {
        ...
        color: limbColor,
        size: [0.02 * scale, 0.18 * scale, 0],
        position: midPt(chestW, midPt(chestW, lhW)),
        velocity: rndV(2, 4),
        rotation: [0, 0, 0],
        angularVel: rndAng(10),
      },
      // Left forearm
      {
        shape: "cylinder",
        color: limbColor,
        size: [0.016 * scale, 0.15 * scale, 0],
        position: midPt(midPt(chestW, lhW), lhW),
        velocity: rndV(2.5, 4),
        rotation: [0, 0, 0],
        angularVel: rndAng(12),
      },
      // Right upper arm
      {
        shape: "cylinder",
        color: limbColor,
        size: [0.02 * scale, 0.18 * scale, 0],
        position: midPt(chestW, midPt(chestW, rhW)),
        velocity: rndV(2, 4),
        rotation: [0, 0, 0],
        angularVel: rndAng(10),
      },
      // Right forearm
      {
        ...
        color: limbColor,
        size: [0.016 * scale, 0.15 * scale, 0],
        position: midPt(midPt(chestW, rhW), rhW),
        velocity: rndV(2.5, 4),
        rotation: [0, 0, 0],
        angularVel: rndAng(12),
      },
      // Left thigh
      {
        ...
        color: legColor,
        size: [0.025 * scale, 0.22 * scale, 0],
        position: midPt(pelvisW, midPt(pelvisW, lfW)),
        velocity: rndV(1, 3),
        rotation: [0, 0, 0],
        angularVel: rndAng(8),
      },
      // Left shin
      {
        ...
        color: legColor,
        size: [0.02 * scale, 0.18 * scale, 0],
        position: midPt(midPt(pelvisW, lfW), lfW),
        velocity: rndV(1.5, 3),
        rotation: [0, 0, 0],
        angularVel: rndAng(10),
      },
      // Right thigh
      {
        ...
        color: legColor,
        size: [0.025 * scale, 0.22 * scale, 0],
        position: midPt(pelvisW, midPt(pelvisW, rfW)),
        velocity: rndV(1, 3),
        rotation: [0, 0, 0],
        angularVel: rndAng(8),
      },
      // Right shin
      {
        ...
        color: legColor,
        size: [0.02 * scale, 0.18 * scale, 0],
        position: midPt(midPt(pelvisW, rfW), rfW),
        velocity: rndV(1.5, 3),
        rotation: [0, 0, 0],
        angularVel: rndAng(10),
      },
      // Left hand
      {
        shape: "sphere",
        color: skinColor,
        size: [0.018 * scale, 0, 0],
        position: lhW,
        velocity: rndV(3, 5),
        rotation: [0, 0, 0],
        angularVel: rndAng(15),
      },
      // Right hand
      {
        shape: "sphere",
  ...
  const triggerFall = useCallback(
    (reason: "reach" | "grip" = "reach") => {
      ...
      const fallAnimate = (now: number) => {
        ...
        setState((prev) => ({
          ...prev,
          lhY: prev.lhY - eased * 0.06,
          rhY: prev.rhY - eased * 0.06,
          lfY: prev.lfY - eased * 0.06,
          rfY: prev.rfY - eased * 0.06,
          leftHandOn: false,
          rightHandOn: false,
          leftFootOn: false,
          rightFootOn: false,
    ...
    [stopSim, buildRagdoll, showClimbingMessage],
  );
  ...
  const loadRoute = useCallback(
    (preset: RoutePreset) => {
      ...
      setWallSegments(
        preset.segments || [{ height: 4, angleDeg: preset.wallAngle }],
      );
      const handUsable = [...holds]
        .filter((h) => h.usage !== "foot")
        .sort((a, b) => a.y - b.y);
      const lh = handUsable[0];
      const rh = handUsable[1] || handUsable[0];
  ...
  const startSim = useCallback(() => {
    ...
    const animate = (now: number) => {
      ...
      while (
        ...
        globalElapsed >=
          schedule[moveIdx].start +
            schedule[moveIdx].duration +
            schedule[moveIdx].pause
      ) {
        moveIdx++;
      }
      ...
      if (
        moveIdx === lastIdx &&
        globalElapsed >=
          schedule[lastIdx].start +
            schedule[lastIdx].duration +
            schedule[lastIdx].pause
      ) {
      ...
      if (moveIdx !== currentMoveIdx) {
        ...
        if (
          !prevMove.isSetup &&
          (prevMove.limb === "leftHand" || prevMove.limb === "rightHand")
        ) {
      ...
      if (!m.isSetup && rawT > 0.1 && rawT < 0.3) {
        ...
        if (isHand) {
          ...
          const armReach =
            (snap2.apeIndexIn / 2) * 0.0254 * (snap2.heightFt / 5.75);
      ...
      if (!m.isSetup && isHand && rawT > 0.85) {
        ...
        const liveConfig: ClimberConfig = {
          ...
          heightFt: s.heightFt,
          apeIndexIn: s.apeIndexIn,
          bodyRotationDeg: s.bodyRotationDeg,
          wallAngleDeg: s.wallAngleDeg,
          leftHandPull: s.leftHandPull,
          rightHandPull: s.rightHandPull,
          leftFootPull: s.leftFootPull,
          rightFootPull: s.rightFootPull,
          leftKneeTurnDeg: s.leftKneeTurnDeg,
          rightKneeTurnDeg: s.rightKneeTurnDeg,
          hipOffset: s.hipOffset,
          torsoOffset: s.torsoOffset,
          leftHandOn: s.leftHandOn,
          rightHandOn: s.rightHandOn,
          leftFootOn: s.leftFootOn,
          rightFootOn: s.rightFootOn,
          leftHand: { x: s.lhX, y: s.lhY },
          rightHand: { x: s.rhX, y: s.rhY },
          leftFoot: { x: s.lfX, y: s.lfY },
          rightFoot: { x: s.rfX, y: s.rfY },
          centerOfGravity: {
            x:
              (s.lfX + s.rfX) / 2 +
              ((s.lhX + s.rhX) / 2 - (s.lfX + s.rfX) / 2) * 0.55,
            y:
              (s.lfY + s.rfY) / 2 +
              ((s.lhY + s.rhY) / 2 - (s.lfY + s.rfY) / 2) * 0.55,
          },
        };
        if (!computeForces(liveConfig).canHold) {
          stopped = true;
          triggerFall("grip");
          return;
        }
      ...
      const pullDir = m.holdType
        ? isHand
          ? holdToPullHand(m.holdType, m.holdDirection)
          : holdToPullFoot(m.holdType, m.holdDirection, isOH)
        : null;
      ...
      setState((prev) => {
        ...
        const targetTwist =
          rawT < 0.15 ? snap.bodyRotationDeg + anticipationTwist : m.bodyTwist;
        ...
        return {
          ...
          torsoOffset: Math.min(
            1,
            lerp(snap.torsoOffset, m.torsoOffset, bodyT) + arcTorsoBonus,
          ),
  ...
  const set = useCallback(
    (key: keyof ClimberState, value: number | string | boolean) => {
  ...
  const applyTwist = useCallback((name: string) => {
    ...
    setState((prev) => ({
      ...prev,
      bodyRotationDeg: t.twist,
      leftKneeTurnDeg: t.lKnee,
      rightKneeTurnDeg: t.rKnee,
    }));
  }, []);
  ...
  const config: ClimberConfig = useMemo(
    () => ({
      ...
      leftFootPull: state.leftFootPull,
      rightFootPull: state.rightFootPull,
      ...
      centerOfGravity: {
        x:
          (state.lfX + state.rfX) / 2 +
          ((state.lhX + state.rhX) / 2 - (state.lfX + state.rfX) / 2) * 0.55,
        y:
          (state.lfY + state.rfY) / 2 +
          ((state.lhY + state.rhY) / 2 - (state.lfY + state.rfY) / 2) * 0.55,
      },
    }),
    [state],
  );
  ...
  return (
    ...
    >
      ...
      <ClimbingScene
        ...
        ragdollCameraTarget={ragdollCameraTarget}
      />

      {/* === CLIMBING IDIOM MESSAGE === */}
      {climbingMessage && (
        <div
          style={{
            position: "absolute",
          ...
          {isFalling ? (
            ...
            >
              ...
              {(() => {
                ...
                const gc =
                  grip > 90 ? "#ff3333" : grip > 60 ? "#ffaa00" : "#44cc66";
                return (
              ...
              >
                {(["left", "right"] as const).map((side) => {
                  ...
                  const color =
                    val > 80 ? "#ff3333" : val > 50 ? "#ffaa00" : "#44cc66";
                  return (
      ...
      {placingMode && !isPlaying && (
        ...
        >
          ...
          >
            {eraserMode
              ? "Tap hold to erase"
              : `Tap wall to place ${HOLD_INFO[selectedHoldType].label}`}
          </div>
        </div>
      )}
      ...
      {!isPlaying && (
        <button
          onClick={() => togglePanel("forces")}
          style={{
            position: "absolute",
            top: 12,
            right: 12,
            zIndex: 20,
            background: forces.canHold
              ? "rgba(0,140,60,0.85)"
              : "rgba(200,30,30,0.85)",
            color: "#fff",
            border: "none",
            ...
            {!isPlaying ? (
              <>
                <button
                  ...
                  style={{
                    ...pill,
                    background: activePanel === "routes" ? "#4488ff" : "#333",
                    flex: 1,
                    maxWidth: 140,
                  }}
                >
                ...
                <button
                  onClick={() => {
                    if (placingMode) {
                      ...
                      setPlacingMode(true);
                      setActivePanel("holds");
                    }
                  }}
                  style={{
                    ...pill,
                    background: placingMode ? "#44aa44" : "#333",
                    flex: 1,
                    maxWidth: 140,
                  }}
                >
                ...
                <button
                  onClick={() => setIsExploring((e) => !e)}
                  style={{
                    ...pill,
                    background: isExploring ? "#44aa44" : "#333",
                    flex: 1,
                    maxWidth: 140,
                  }}
                >
                ...
                <button
                  ...
                  style={{
                    ...pill,
                    background: activePanel === "settings" ? "#4488ff" : "#333",
                    width: 44,
                    fontSize: 18,
                  }}
                >
                ...
                {placedHolds.length > 0 && (
                  <button
                    ...
                    style={{
                      ...pill,
                      background: showCopied ? "#44aa44" : "#555",
                      flex: 1,
                      maxWidth: 100,
                    }}
                  >
          ...
          {placingMode && !isPlaying && (
            ...
            >
              <button
                ...
                style={{
                  ...chip,
                  background: eraserMode ? "#cc6644" : "#333",
                  fontSize: 12,
                }}
              >
              ...
              {placedHolds.length > 0 && (
                <button onClick={undoHold} style={{ ...chip, fontSize: 12 }}>
                  Undo
                </button>
              )}
              <button onClick={resetClimber} style={{ ...chip, fontSize: 12 }}>
                Reset
              </button>
              {placedHolds.length > 0 && (
                <button
                  onClick={clearHolds}
                  style={{ ...chip, fontSize: 12, color: "#ff6666" }}
                >
                  Clear All
                </button>
              )}
      ...
      {activePanel !== "none" && !isPlaying && (
        <div
          style={{
            position: "absolute",
            bottom: placingMode ? 175 : placedHolds.length > 0 ? 155 : 115,
            left: "50%",
            transform: "translateX(-50%)",
            width: "100%",
            maxWidth: 520,
                ...
                {(["both", "hand", "foot"] as HoldUsage[]).map((u) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (
                        u === "foot" &&
                        !["foot-chip", "foot-edge", "smear-pad"].includes(
                          selectedHoldType,
                        )
                      )
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        minWidth: 28,
                        background:
                          selectedDirection === d ? "#4488ff" : "#2a2a35",
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedUsage(info.defaultUsage);
                      }}
                      style={{
                        padding: "4px 8px",
                        background:
            ...
            <div>
              ...
              >
                <Slider
                  ...
                  suffix={`kg / ${(state.bodyWeightKg * 2.20462).toFixed(0)} lbs`}
                />
                <Slider
                  label="Grip"
                  value={state.gripStrengthKg}
                  min={10}
                  max={100}
                  step={1}
                  onChange={(v) => set("gripStrengthKg", v)}
                  suffix={`kg / ${(state.gripStrengthKg * 2.20462).toFixed(0)} lbs`}
                />
                <Slider
                  label="Height"
                ...
                >
                  {feetToFtIn(state.heightFt)} /{" "}
                  {(state.heightFt * 30.48).toFixed(0)}cm
                </div>
                <Slider
                  label="Ape Index"
                  value={state.apeIndexIn}
                  min={54}
                  max={84}
                  step={0.5}
                  onChange={(v) => set("apeIndexIn", v)}
                  suffix='"'
                />
              </div>
              ...
              >
                {wallSegments.map((seg, i) => (
                  ...
                  >
                    ...
                    >
                      <span
                        style={{ fontSize: 11, color: "#aaa", fontWeight: 600 }}
                      >
                        Segment {i + 1}
                      </span>
                    ...
                    <Slider
                      ...
                      onChange={(v) => {
                        ...
                        const newSegs = wallSegments.map((s, j) =>
                          j === i ? { ...s, angleDeg: v } : s,
                        );
                ...
                <button
                  onClick={() => {
                    const lastAngle =
                      wallSegments[wallSegments.length - 1].angleDeg;
                    setWallSegments((prev) => [
                      ...prev,
                      { height: 2, angleDeg: lastAngle },
                    ]);
                  }}
                  style={{
                    width: "100%",
              ...
              >
                <Slider
                  label="Twist"
                  value={state.bodyRotationDeg}
                  min={-90}
                  max={90}
                  step={1}
                  onChange={(v) => set("bodyRotationDeg", v)}
                  suffix="\u00b0"
                />
                <div
                  style={{
                  ...
                  {Object.keys(TWIST_PRESETS).map((name) => (
                    <button
                      ...
                      style={{
                        ...
                        padding: "4px 8px",
                        background:
                          activeTwist === name ? "#4488ff" : "#2a2a35",
                      }}
                    >
                      {name}
                    </button>
                ...
                <Slider
                  label="Hip Dist"
                  value={state.hipOffset}
                  min={0}
                  max={1}
                  step={0.05}
                  onChange={(v) => set("hipOffset", v)}
                />
                <Slider
                  label="Torso"
                  value={state.torsoOffset}
                  min={0}
                  max={1}
                  step={0.05}
                  onChange={(v) => set("torsoOffset", v)}
                />
              </div>
              ...
              <div
                style={{ background: "#1a1a22", borderRadius: 10, padding: 12 }}
              >
                <div
                  style={{
                    fontSize: 12,
                ...
                >
                  Left Hand
                  <button
                    onClick={() => set("leftHandOn", !state.leftHandOn)}
                    style={{
                      fontSize: 10,
                      padding: "2px 6px",
                      borderRadius: 4,
                      border: "1px solid #555",
                      background: state.leftHandOn ? "#4a4" : "#a44",
                      color: "#fff",
                      cursor: "pointer",
                    }}
                  >
                    {state.leftHandOn ? "On" : "Off"}
                  </button>
                </div>
                <div
                  style={{
                ...
                >
                  Right Hand
                  <button
                    onClick={() => set("rightHandOn", !state.rightHandOn)}
                    style={{
                      fontSize: 10,
                      padding: "2px 6px",
                      borderRadius: 4,
                      border: "1px solid #555",
                      background: state.rightHandOn ? "#4a4" : "#a44",
                      color: "#fff",
                      cursor: "pointer",
                    }}
                  >
                    {state.rightHandOn ? "On" : "Off"}
                  </button>
                </div>
                <div
                  style={{
                ...
                >
                  Left Foot
                  <button
                    onClick={() => set("leftFootOn", !state.leftFootOn)}
                    style={{
                      fontSize: 10,
                      padding: "2px 6px",
                      borderRadius: 4,
                      border: "1px solid #555",
                      background: state.leftFootOn ? "#4a4" : "#a44",
                      color: "#fff",
                      cursor: "pointer",
                    }}
                  >
                    {state.leftFootOn ? "On" : "Off"}
                  </button>
                </div>
                <div
                  style={{
                ...
                >
                  <select
                    value={state.leftFootPull}
                    onChange={(e) => set("leftFootPull", e.target.value)}
                    style={{
                      flex: 1,
                      background: "#333",
                      color: "#eee",
                ...
                <Slider
                  ...
                  max={2}
                  step={0.05}
                  onChange={(v) => set("lfY", v)}
                />
                <Slider
                  label="Knee"
                  value={state.leftKneeTurnDeg}
                  min={-90}
                  max={90}
                  step={1}
                  onChange={(v) => set("leftKneeTurnDeg", v)}
                  suffix="\u00b0"
                />
                ...
                >
                  Right Foot
                  <button
                    onClick={() => set("rightFootOn", !state.rightFootOn)}
                    style={{
                      fontSize: 10,
                      padding: "2px 6px",
                      borderRadius: 4,
                      border: "1px solid #555",
                      background: state.rightFootOn ? "#4a4" : "#a44",
                      color: "#fff",
                      cursor: "pointer",
                    }}
                  >
                    {state.rightFootOn ? "On" : "Off"}
                  </button>
                </div>
                <div
                  style={{
                ...
                >
                  <select
                    value={state.rightFootPull}
                    onChange={(e) => set("rightFootPull", e.target.value)}
                    style={{
                      flex: 1,
                      background: "#333",
                      color: "#eee",
                ...
                <Slider
                  ...
                  max={2}
                  step={0.05}
                  onChange={(v) => set("rfY", v)}
                />
                <Slider
                  label="Knee"
                  value={state.rightKneeTurnDeg}
                  min={-90}
                  max={90}
                  step={1}
                  onChange={(v) => set("rightKneeTurnDeg", v)}
                  suffix="\u00b0"
                />
              </div>
            </div>
